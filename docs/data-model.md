...
/ Habits list, and the Workout list — but only where a colour is actually
set; an unset category keeps its built-in look (`colorForCategorySlot`'s
...
| `habit_reminders` | Manage → Visible sections: an optional daily `reminder_time` per (`user_id`, `domain`) — one of the seven `TrackedDomain`s (`food` / `outcome` / `supplement` / `habit` / `stool` / `workout` / `cycle`). Same `reminder_time` / `reminder_last_sent_date` shape as `supplement_items`/`habit_items`, but resolved once anything in that domain is logged that day, not any one item. Row absence = no reminder for that domain. |
