...
(`SegmentedTabs`; equal-width segments when they fit, otherwise natural widths with the spare space shared evenly, and only what genuinely doesn't fit folds into "More"); section switchers one
...
|---|---|---|
| **Log** | `/log` | Tap-to-log entry for the eight tracking domains plus a Summary tab (searching a name that isn't tracked offers to add it): Food, Symptoms, Supplements, Habits, Stool, Workout, Cycle, Coffee, Summary. Food, Symptoms, Supplements and Habits share one layout: search, meal/time-of-day and time on a single row (on desktop that row moves up beside the page title, so the domain tabs get a full row of their own), then the categories as one grouped list of tappable rows (collapsible on mobile; short Symptoms/Supplements/Habits lists stay open; a column grid from `lg` up). Food replaces the collapsible list with a scrolling category strip and one full alphabetical list below it for the selected category (list rows on phones; from `lg` up, as many columns as fit, with tighter rows under a mouse), with a first "Usual" tab of what you log most at the selected meal and a "Sep picks" tab (short month name) of in-season foods you haven't eaten lately. From `sm` up each category tab shows its icon (the custom one set in Settings, else Food's built-in glyph). A logged item shows as tinted text with a ✓. Food also adds a "Products" row above it — a saved product (e.g. a bought smoothie) logs every one of its ingredients at once. Searching a name that isn't tracked can also add it as a product right there — name, optional brand and ingredients (any not yet tracked are created) — and logs it straight away; search matches saved products too. Coffee is grouped by brand: searching a new name adds it with an optional brand and notes, and tapping any coffee opens a per-cup form (café, price, brewing type/method, water temp, tasting characteristics). Summary is the day's read-back: today's meals grouped with their notes, then everything logged across every domain in one list grouped under the hour it happened, each entry colour-coded by domain and still fully editable (time, meal/time-of-day, value, note) and deletable from there. |
...
- **Colours / branding** — CSS variables in `src/app/globals.css`. `--brand-*` is
  ...
  Toolbar controls — `SearchField`, `Button variant="outline"`, Log's meal/time menus, the date stepper, the Filter and date-range buttons, the header menu button — share one raised shape (`CONTROL_CLS` in `ui/Chip.tsx`: 36px, 10px radius, the `.control-surface` white fill with a hairline edge and soft shadow from `globals.css`); menus show their value in the tint with an up/down chevron. `SegmentedTabs` and `Segmented` put that same raised surface on the active segment, over a faint `--segment-track` wash. Forms are iOS grouped rows: `FormShell` (title + Cancel) holds `FormGroup` cards of `Field` rows — label above the value, or `inline` with the value right-aligned — and `SwitchRow` for toggles, with no boxed inputs. Dates and times are never native inputs: `ui/DatePicker.tsx` (`DatePicker`, `TimePicker`, `DateTimePicker`, `MonthPicker`) shows the value in a raised capsule ("20 Sep 2026", "10:48", or "None") and opens a calendar and/or hour-minute wheels in a `Sheet`, passing ISO strings in and out. Compact controls extend to a 44px hit area (`.hit-slop`), keyboard focus shows an accent ring, and `--text-muted` is held at about 4.5:1. Dialogs are `Sheet` (a bottom sheet floating 8px in from the screen edges on phones, centred from `sm`, 20px corners); `DuplicateItemDialog` is the iOS-style alert with side-by-side buttons. Popover menus (date range, "More", date pickers, suggestion lists) share the `.menu-surface` panel from `globals.css`: white, a hairline edge, a deeper drop shadow, and plain 40px rows with the chosen one tinted. Filters and category switchers that change what a list shows are `TabRail` text tabs (`tall`), not pills; two-way modes use `Segmented`. Small metadata (kind, specialty, "shared") is plain text, never a badge. `ListSection` is an iOS section header above a white card. Cards and grouped lists use `rounded-xl`, set to 16px in `globals.css`; controls and buttons keep a 10px radius.
