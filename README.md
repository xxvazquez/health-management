...
| **Health** | `/medical` | Four tabs. **Visits** — two sections: "Before your next visit" (upcoming appointment dates + the dated observations/notes waiting for a visit, filterable by specialty) and "Past visits" (appointments already attended, each with its follow-up tasks inline). **Results** — an Overview that is the marker list: every marker on a reference-range bar with the optimal band marked, grouped by panel or sorted A–Z, with a panel filter (chips that scroll sideways) to narrow to one panel without scrolling, and a time-window control (All / 5y / 2y / 1y) that also switches each bar between the window average (its lowest-to-highest spread drawn as a whisker) and the latest reading. Tapping a marker opens its trend chart (a real time axis showing every year, or every month, in the window — labels turned vertical when tight), window stats (latest, previous, change, average, spread, draw count) and full value history with notes, where you also add or edit its values (a single reading or a whole-draw batch). A new marker gets a slim quick-add (name, unit, panel) here; markers and panels — reference and optimal ranges, renames, icons/colours, grouping, delete — are managed in Settings. Each marker can carry an optional optimal range, tighter than the lab reference range — values are read against it first, so a result inside the lab range but below your target reads as "below optimal" (a marker with no optimal range falls back to the lab norm and reads "below/above norm"). The reading is coloured green in band, red out. Absorbed the old Trends → Blood dashboard. **Vitals** — blood pressure and weight with trend charts (a 1m / 3m / 6m / 1y / All time-window control that pins the x-axis to the window, so every month or year in it shows even when readings are sparse) and ACC/AHA categories, plus an optional weight-goal band on the weight chart (the goal range is set in Settings; Vitals shows it read-only). **Doctors** — read-only: picking a doctor shows their rating/language/specialty and visit history in place on mobile, in a side pane on desktop. All doctor editing (name, rating, language, specialty, add, delete) and doctor-type rename/archive live in Settings. `/doctors` redirects here; old tab hashes (`#appointments`, `#carelog`, `#followups`, `#specialties`) land on Visits. |
...
| **Messages** | `/notes` | Primary nav, partner-linked only — a link in the sidebar / mobile menu drawer (a dot on the menu button carries the unread cue), never the bottom bar. Private one-to-one messaging with your linked partner. |
| Settings | `/manage` | (Sidebar foot.) Add / rename / archive / delete items and categories, give a category its own icon/colour, set exercise units, correct a food's automatic nutrition-group classification, edit reminder lists, wishlist lists (name/icon/colour), the Vitals weight goal, lab markers and panels (ranges, units, grouping, icons/colours), doctors (name/rating/language/specialty), doctor types and the Stool tab's colour/symptom/floatation/characteristic chips, show or hide tracked sections (they otherwise appear once they have data), and export your data (whole account as JSON, or a section — or everything — as CSV in one file). Searchable across every section. Also linked from Log's inline "add item". |
...
- **Medical** (`doctor_specialties` / `doctors` / `doctor_appointments` /
  ...
  Visits' "Before your next visit" section, filterable by specialty. An entry may
...
- **Colours / branding** — CSS variables in `src/app/globals.css`. `--brand-*` is
  ...
  Light / Dark / System choice (Settings → Appearance; `src/lib/theme.ts`,
