...
| **Trends** | `/analytics` | An **Overview** tab (today's story + what stands out across every domain + a week/month review — moved off Agenda) then one dashboard per Log domain: Food, Supplements, Habits, Stool, Workout, Cycle, and **Patterns**. Switched by a tab bar. (Blood/lab analysis moved to Health → Results; `/analytics#labs` redirects there.) |
