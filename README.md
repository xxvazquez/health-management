...
|---|---|---|
| **Log** | `/log` | Tap-to-log entry for the eight tracking domains (searching a name that isn't tracked offers to add it): Food, Symptoms, Supplements, Habits, Stool, Workout, Cycle, Coffee. Food's own tab lists its categories in one grouped list, with a "Products" row above it — a saved product (e.g. a bought smoothie) logs every one of its ingredients at once. Searching a name that isn't tracked can also add it as a product right there — name, optional brand and ingredients (any not yet tracked are created) — and logs it straight away; search matches saved products too. Coffee is grouped by brand, with a per-cup form (café, price, brewing type/method, water temp, tasting characteristics). |
