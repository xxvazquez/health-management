  ...
  return (
    <FormShell title="New product" onSubmit={handleSubmit} onCancel={onCancel} submitLabel="Add" submitDisabled={!canSave} busy={busy} accent={accent}>
      <FormGroup>
        ...
        <Field label={<>Brand <span style={{ color: "var(--text-muted)" }}>· optional</span></>}>
          <input value={brand} onChange={(e) => setBrand(e.target.value)} maxLength={120} placeholder="e.g. Maczfit" className={ROW_TEXT_CLS} style={ROW_STYLE} />
        </Field>
      </FormGroup>
      ...
      <FormGroup title="Ingredients" footer="Anything you haven't tracked yet is added to your foods automatically.">
        ...
        {suggestions.length > 0 && (
          <div className="flex flex-wrap gap-1.5 px-3.5 py-2.5">
            {suggestions.map((s) => (
              <Chip key={s} onClick={() => addIngredient(s)}>
                + {s}
              </Chip>
            ))}
          </div>
