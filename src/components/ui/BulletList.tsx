...
export function BulletList({
  ...
  bullets,
  emptyText,
  /** Render each bullet's label as a lowercase italic term rather than a
   * bold noun — used on the Food page, where the labels are taxonomy group
   * names ("other veg", "citrus fruit") read inside a sentence. */
  termLabels = false,
}: {
  title: string;
  tone?: string;
          ...
          {bullets.map((b) => (
            <li key={b.label} className="flex gap-2.5">
              ...
              <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
                <span
                  className={termLabels ? "italic lowercase" : "font-medium"}
