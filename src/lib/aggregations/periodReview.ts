...
export function buildPeriodReview(
  ...
  if (totals.symptomDays > 0) {
    highlights.push(`Symptoms logged on ${totals.symptomDays} day${totals.symptomDays === 1 ? "" : "s"}.`);
  }
  if (totals.workoutSessions > 0) {
    highlights.push(`Trained on ${totals.workoutSessions} day${totals.workoutSessions === 1 ? "" : "s"}.`);
  }
  if (totals.periodDays > 0) {
    highlights.push(`${totals.periodDays} period day${totals.periodDays === 1 ? "" : "s"} logged.`);
  }
